...
exports.getUser = async (req, res) => {
  ...
  try {
    const [user] = db.execute("SELECT name, email FROM users WHERE email = ?", [
      email,
    ]);
    if (user.length === 0) {
      return res.status(404).json({ message: "The user does not exist" });
    ...
    return res
      .status(200)
      .json({ message: "Details fetched for editing!", user: user[0] });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
...
exports.editUser = async (req, res) => {
  ...
  try {
    await db.execute(
      ...
      [name, email, hashedPashword, req.session.userId]
    );
    return res
      .status(200)
